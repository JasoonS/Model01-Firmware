...
enum { PDVORAK, NUMPAD, FUNCTION, STENO }; // layers
...
KEYMAPS(
  ...
  (M(MACRO_PD_TILDE), M(MACRO_PD_ONE), M(MACRO_PD_TWO), M(MACRO_PD_THREE), M(MACRO_PD_FOUR), M(MACRO_PD_FIVE),     Key_PrintScreen,
   ...
   Key_LeftShift,   Key_Backspace,   Key_Tab,                 Key_Home,
   ShiftToLayer(NUMPAD),
   ...
   Key_End,         Key_Enter,       Key_Spacebar,     Key_LeftControl,
   LockLayer(FUNCTION)),
  ...
  (UnlockLayer(STENO),    XXX,   XXX,   XXX,   XXX,   XXX,   S(N6),
